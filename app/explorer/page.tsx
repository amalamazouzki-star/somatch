...
const creators = [
  { name:"Hafsa Achraf", image:"/explorer/maya.png", niche:"Lifestyle · Méga", city:"Casablanca, Maroc", score:92, instagram:"1,4 M", tiktok:"620K", youtube:"–", engagement:"5,2 %", views:"620K", match:"Excellent match", budget:"22 000 MAD", verified:"green" },
...
function FilterSection({ index, title, children, open, onToggle }: { index:number; title:string; children:React.ReactNode; open:boolean; onToggle:()=>void }) {
  return <section className={`advanced-drawer-section ${open ? "open" : "closed"}`}><button className="advanced-section-title" type="button" onClick={onToggle} aria-expanded={open}><strong>{index}. {title}</strong><span>{open ? "⌃" : "⌄"}</span></button>{open ? <div className="advanced-section-body">{children}</div> : null}</section>;
}
  ...
  return <main className="dashboard-page advanced-explorer-page">
    ...
    <div className="advanced-explorer-workspace">
      <section className="advanced-explorer-content">
        <header className="advanced-explorer-header"><div><h1>Explorer les influenceurs</h1><p>Trouvez les créateurs parfaits pour votre prochaine campagne.</p></div><div><button className={savedSearch ? "is-saved" : ""} type="button" onClick={()=>setSavedSearch(value=>!value)}>▯　{savedSearch ? "Recherche enregistrée ✓" : "Enregistrer la recherche"}</button><button className="open-advanced-filters" type="button" onClick={()=>setDrawerOpen(true)}>☷　Filtres avancés <b>{advancedFilterCount}</b></button></div></header>
        ...
        <div className="advanced-results-toolbar"><strong>{resultCount} {resultLabel}</strong><div><label>Trier par : <select defaultValue="Pertinence"><option>Pertinence</option><option>Engagement</option><option>SoMatch Score</option></select></label><span className="advanced-view-toggle"><button className={gridView?"active":""} type="button" onClick={()=>setGridView(true)} aria-label="Afficher en grille" aria-pressed={gridView}>⊞</button><button className={!gridView?"active":""} type="button" onClick={()=>setGridView(false)} aria-label="Afficher en liste" aria-pressed={!gridView}>☷</button></span></div></div>
      ...
      <aside className={`advanced-filter-drawer ${drawerOpen?"open":"closed"}`} aria-hidden={!drawerOpen}>
        ...
        <div className="advanced-drawer-scroll">
          ...
          <FilterSection index={2} title="Réseaux & communauté" open={openSections.includes(2)} onToggle={()=>toggleSection(2)}><div className="platform-choice"><span>Plateformes</span><div>{["Instagram","TikTok","YouTube"].map(item=><label className={platforms.includes(item)?"checked":""} key={item}><input type="checkbox" checked={platforms.includes(item)} onChange={()=>toggleArray(item,platforms,setPlatforms)}/>{item}</label>)}</div></div><RangeControl label="Abonnés (réseau principal)" min="10K" max="5M+" progress="88%"/></FilterSection>
          ...
          <FilterSection index={4} title="Audience" open={openSections.includes(4)} onToggle={()=>toggleSection(4)}><div className="drawer-two-columns"><RangeControl label="Âge moyen des abonnés" min="18" max="35" progress="78%"/><RangeControl label="Genre des abonnés" min="60 % Femmes" max="100 %" progress="62%"/><SelectField label="Pays / villes des abonnés" value="Maroc"/><div className="add-city"><span>&nbsp;</span><button type="button">＋ Ajouter une ville</button></div></div></FilterSection>
          <FilterSection index={5} title="Qualité SoMatch" open={openSections.includes(5)} onToggle={()=>toggleSection(5)}><RangeControl label="SoMatch Score minimum" min="" max="80" progress="78%" dual={false}/><div className="drawer-two-columns quality-selects"><SelectField label="Authenticité de l’audience" value="Élevée à très élevée"/><SelectField label="Affinité avec la marque" value="Bonne à excellente"/></div></FilterSection>
        </div>
